...
def kmean():
    ...
    apdata = apdata_prepare(ospJoin(
        BASE_DIR, 'home/logic/CSVs/new_purchase.csv'), km.labels_, df['CustomerID'])

    labels = append(km.labels_, [(6)
                                 for i in range(len(km.cluster_centers_))], axis=0)

    clusterInfo = kmeansAttrs.append(pd.DataFrame(
        km.cluster_centers_.tolist(), columns=["Age", "Annual_income", "Spending_score"]))
...
def apr(clust_label, ms, mc, ml):
    ar = apriori(apdata[clust_label], min_support=ms,
...
def giveAssocOnAgeRange(age_min, age_max):
    ...
    df_trans = pd.read_csv(
        ospJoin(BASE_DIR, 'home/logic/CSVs/new_purchase.csv'), header=None)
    sid = extract_ids(df['Age'].tolist(),
                      df['CustomerID'].tolist(), age_min, age_max)
    list_of_transactions = extract_transactions(df_trans, sid)
    ar = apriori(list_of_transactions, min_support=0.004,
                 min_confidence=0.5, min_lift=7)
    results = list(ar)
    d = {}
    i = 0
    for record in results:
